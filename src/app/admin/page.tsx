...
type Tab = "dashboard" | "bookings" | "messages" | "equipment" | "blacklist";
...
export default function AdminDashboard() {
  ...
  const exportCSV = () => {
    const headers = ["Name", "Email", "Phone", "Instagram", "Equipment", "Pickup", "Return", "Status", "Payment", "Amount"];
    const rows = bookings.map(b => [b.name, b.email, b.phone, b.instagram, b.equipment, b.pickup_date, b.return_date, b.status, b.payment_method, b.total_amount || ""]);
  ...
  const totalRevenue = bookings.filter(b => b.status === "completed").reduce((acc, b) => acc + (b.total_amount || 0), 0);
    ...
    <div className="min-h-screen bg-gray-900 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 border-r border-gray-700 transform transition-transform lg:translate-x-0 lg:static ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-gray-700">
          <button onClick={handleLogout} className="w-full px-3 py-2 text-sm text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
            Sign Out
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 min-h-screen">
        <header className="sticky top-0 z-30 bg-gray-900/95 backdrop-blur border-b border-gray-700 px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          <button onClick={exportCSV} className="px-3 py-1.5 bg-gray-700 text-gray-300 text-xs rounded-lg hover:bg-gray-600 transition-colors">
            Export CSV
          </button>
        </header>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                  <p className="text-gray-400 text-xs">Total Revenue (Completed)</p>
              ...
              <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-700">
                  <h3 className="text-white font-medium text-sm">Recent Bookings</h3>
                </div>
                  ...
                  {bookings.slice(0, 5).map(b => (
                    <div key={b.id} className="px-4 py-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-gray-400 text-xs">{b.equipment} • {b.pickup_date}</p>
                      </div>
                      <span className={`px-2 py-0.5 text-xs rounded-full ${b.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : b.status === "confirmed" ? "bg-blue-500/20 text-blue-400" : b.status === "completed" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
                        {b.status}
                      </span>
                    </div>
                  ))}
              ...
              {bookings.map(b => (
                <div key={b.id} className="bg-gray-800 rounded-xl border border-gray-700 p-4 sm:p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                    ...
                    <span className={`self-start px-3 py-1 text-xs rounded-full font-medium ${b.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : b.status === "confirmed" ? "bg-blue-500/20 text-blue-400" : b.status === "completed" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
                      {b.status}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm mb-4">
                    ...
                    <div><p className="text-gray-500 text-xs">Payment</p><p className="text-gray-200">{b.payment_method}</p></div>
                  </div>
                  {b.purpose && <p className="text-gray-400 text-sm mb-3">Purpose: {b.purpose}</p>}
                  <div className="flex flex-wrap gap-2 mb-3">
                    {b.id_photo_url && b.id_photo_url !== "Not uploaded" && (
                      <a href={b.id_photo_url} target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-gray-700 text-gray-300 text-xs rounded-lg hover:bg-gray-600">📎 View ID</a>
                    )}
                    {b.receipt_url && b.receipt_url !== "Not uploaded" && (
                      <a href={b.receipt_url} target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-gray-700 text-gray-300 text-xs rounded-lg hover:bg-gray-600">📎 View Receipt</a>
                    )}
                  </div>
                ...
                <div key={m.id} className={`bg-gray-800 rounded-xl border p-4 ${m.is_read ? "border-gray-700" : "border-[#C5A044]/30"}`}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-1">
                      {!m.is_read && (
                        <button onClick={() => markMessageRead(m.id)} className="px-2 py-1 bg-gray-700 text-gray-400 text-xs rounded hover:bg-gray-600">✓</button>
                      )}
